...
- Video semigraphics and attribute edge cases need broader app/game validation.
- Game compatibility matrix is not complete.
- Game audio is verified with SCARFMAN, but needs broader testing.
- Optional disk seek/motor sound effects are planned but not implemented.
...
- Run `BACKUP :0 :1` far enough to confirm expected prompts/behavior.
